...
class Lexer{
  constructor(src){this.src=src;this.pos=0;this.line=1;this.tokens=[];this._run();}
...
class Proto{
  ...
  p(pc,f,v){this.code[pc][f]=v;}
  aL(name){const reg=this._reg++;this._locals.push({name,reg});return reg;}
  fL(name){for(let i=this._locals.length-1;i>=0;i--)if(this._locals[i].name===name)return this._locals[i].reg;return-1;}
  fU(name){const i=this.upvals.indexOf(name);if(i>=0)return i;if(this.parent){const pr=this.parent.fL(name);if(pr>=0){this.upvals.push(name);return this.upvals.length-1;}const pu=this.parent.fU(name);if(pu>=0){this.upvals.push(name);return this.upvals.length-1;}}return-1;}
}
class Compiler{
  constructor(){this.proto=null;this.tokens=[];this.pos=0;}
  compile(src){const lex=new Lexer(src);this.tokens=lex.tokens;this.pos=0;this.proto=new Proto();this.proto.isVararg=true;this._blk();this._exp(TK.EOF);this.proto.e(OP.RETURN,0,1,0);return this.proto;}
  pk(){return this.tokens[this.pos]||{t:TK.EOF,v:''};}
  av(){return this.tokens[this.pos++]||{t:TK.EOF,v:''};}
  ck(t){return this.pk().t===t;}
  mt(...ts){if(ts.includes(this.pk().t))return this.av();return null;}
  _exp(t){const tk=this.av();if(tk.t!==t)throw new Error(`Expected ${t} got ${tk.t}('${tk.v}')`);return tk;}
  _blk(){while(true){const t=this.pk().t;if(t===TK.EOF||t==='end'||t==='else'||t==='elseif'||t==='until')break;if(t==='return'){this._ret();break;}this._stat();}}
  _stat(){const t=this.pk().t;if(t===';'){this.av();return;}if(t==='if')return this._if();if(t==='while')return this._while();if(t==='do'){this.av();this._blk();this._exp('end');return;}if(t==='for')return this._for();if(t==='repeat')return this._repeat();if(t==='function')return this._fnStat();if(t==='local')return this._local();if(t==='goto'||t===TK.DCOLON){this.av();this.av();if(t===TK.DCOLON)this._exp(TK.DCOLON);return;}if(t==='break'){this.av();return;}this._exStat();}
  _ret(){this.av();const base=this.proto._reg;let n=0;if(!['end','else','elseif','until',TK.EOF].includes(this.pk().t)){n=this._eList(base);}this.proto.e(OP.RETURN,base,n+1,0);this.mt(';');}
  _if(){this._exp('if');const base=this.proto._reg;this._cj(base);const jp=this.proto.code.length-1;this._exp('then');this._blk();const ej=[];while(this.ck('elseif')||this.ck('else')){if(this.ck('else')){ej.push(this.proto.e(OP.JMP,0,0,0));this.proto.p(jp,'b',this.proto.code.length-jp-1);this.av();this._blk();break;}else{ej.push(this.proto.e(OP.JMP,0,0,0));this.proto.p(jp,'b',this.proto.code.length-jp-1);this.av();const b2=this.proto._reg;this._cj(b2);const jp2=this.proto.code.length-1;this._exp('then');this._blk();ej.push(this.proto.e(OP.JMP,0,0,0));this.proto.p(jp2,'b',this.proto.code.length-jp2-1);}}this._exp('end');const here=this.proto.code.length;ej.forEach(pc=>this.proto.p(pc,'b',here-pc-1));if(this.proto.code[jp].b===0)this.proto.p(jp,'b',here-jp-1);}
  _cj(base){this._expr(base);this.proto.e(OP.TEST,base,0,0);this.proto.e(OP.JMP,0,0,0);}
  _while(){this._exp('while');const top=this.proto.code.length;const base=this.proto._reg;this._cj(base);const jp=this.proto.code.length-1;this._exp('do');this._blk();this._exp('end');const back=top-this.proto.code.length-1;this.proto.e(OP.JMP,0,back,0);this.proto.p(jp,'b',this.proto.code.length-jp-1);}
  _for(){this._exp('for');const name=this._exp(TK.NAME).v;if(this.ck(TK.ASSIGN)){this.av();const rb=this.proto._reg;this._expr(rb);this.proto._reg++;this._exp(',');this._expr(rb+1);this.proto._reg++;if(this.mt(',')){this._expr(rb+2);this.proto._reg++;}else{this.proto.e(OP.LOADK,rb+2,-1-this.proto.aC(1),0);this.proto._reg++;}const pp=this.proto.e(OP.FORPREP,rb,0,0);this.proto._locals.push({name,reg:rb+3});this._exp('do');this._blk();this._exp('end');const lp=this.proto.e(OP.FORLOOP,rb,0,0);this.proto.p(pp,'b',lp-pp-1);this.proto.p(lp,'b',pp-lp);this.proto._locals.pop();this.proto._reg=rb;}else{this._exp('in');const rb=this.proto._reg;this._eList(rb);this._exp('do');this._blk();this._exp('end');}}
  _repeat(){this._exp('repeat');const top=this.proto.code.length;this._blk();this._exp('until');const base=this.proto._reg;this._expr(base);this.proto.e(OP.TEST,base,0,1);this.proto.e(OP.JMP,0,top-this.proto.code.length-1,0);}
  _fnStat(){this._exp('function');const name=this._exp(TK.NAME).v;const reg=this.proto._reg++;this._fnBody(reg);const k=this.proto.aC(name);this.proto.e(OP.SETGLOBAL,reg,k,0);this.proto._reg--;}
  _local(){this.av();if(this.ck('function')){this.av();const name=this._exp(TK.NAME).v;const reg=this.proto.aL(name);this._fnBody(reg);}else{const names=[];names.push(this._exp(TK.NAME).v);while(this.mt(','))names.push(this._exp(TK.NAME).v);const base=this.proto._reg;if(this.mt(TK.ASSIGN))this._eList(base);names.forEach((n,i)=>{if(this.proto._reg<base+i+1){this.proto.e(OP.LOADNIL,base+i,base+i,0);this.proto._reg++;}this.proto._locals.push({name:n,reg:base+i});});this.proto._reg=base+names.length;}}
  _exStat(){const base=this.proto._reg;this._sfxExpr(base);if(this.ck(TK.ASSIGN)||this.ck(',')){const tgts=[base];while(this.mt(',')){this.proto._reg++;this._sfxExpr(this.proto._reg-1);tgts.push(this.proto._reg-1);}this._exp(TK.ASSIGN);const vb=this.proto._reg;this._eList(vb);tgts.forEach((t,i)=>this.proto.e(OP.MOVE,t,vb+i,0));this.proto._reg=base;}else this.proto._reg=base;}
  _expr(dest){return this._bin(dest,0);}
  _bin(dest,mp){
    const PR={[TK.OR]:1,[TK.AND]:2,[TK.LT]:3,[TK.GT]:3,[TK.LE]:3,[TK.GE]:3,[TK.EQ]:3,[TK.NEQ]:3,[TK.CONCAT]:4,[TK.PLUS]:5,[TK.MINUS]:5,[TK.STAR]:6,[TK.SLASH]:6,[TK.PERCENT]:6,[TK.CARET]:7};
    const OM={[TK.PLUS]:OP.ADD,[TK.MINUS]:OP.SUB,[TK.STAR]:OP.MUL,[TK.SLASH]:OP.DIV,[TK.PERCENT]:OP.MOD,[TK.CARET]:OP.POW,[TK.CONCAT]:OP.CONCAT,[TK.LT]:OP.LT,[TK.GT]:OP.LT,[TK.LE]:OP.LE,[TK.GE]:OP.LE,[TK.EQ]:OP.EQ,[TK.NEQ]:OP.EQ};
    this._un(dest);
    while(true){const op=this.pk().t;const pr=PR[op];if(!pr||pr<=mp)break;this.av();const rhs=this.proto._reg++;this._bin(rhs,pr);const lo=OM[op]??OP.ADD;const fl=(op===TK.GT||op===TK.GE)?1:0;const ng=(op===TK.NEQ)?1:0;if(lo===OP.EQ||lo===OP.LT||lo===OP.LE){this.proto.e(lo,ng,fl?rhs:dest,fl?dest:rhs);this.proto.e(OP.JMP,0,1,0);this.proto.e(OP.LOADBOOL,dest,0,1);this.proto.e(OP.LOADBOOL,dest,1,0);}else this.proto.e(lo,dest,dest,rhs);this.proto._reg--;}
  }
  _un(dest){const t=this.pk().t;if(t===TK.MINUS){this.av();this._un(dest);this.proto.e(OP.UNM,dest,dest,0);}else if(t==='not'){this.av();this._un(dest);this.proto.e(OP.NOT,dest,dest,0);}else if(t===TK.HASH){this.av();this._un(dest);this.proto.e(OP.LEN,dest,dest,0);}else this._sExpr(dest);}
